...
test.describe("Scenario Sidebar", () => {
  ...
  test("workflow limits move to the first chat section instead of the rail", async ({ page }) => {
    ...
    await expect(page.getByTestId("workflow-brief")).toContainText("No Claim/ExplanationOfBenefit resources")
  ...
  test("workflow action chips keep a stable anchor point across cards", async ({
    page,
  }) => {
