...
export function buildIdleSpec(workflows: Workflow[]): RenderTreeNode {
  return node("Stack", { direction: "vertical", gap: "sm" }, [
    node("Heading", {
      text: "Ask the data",
      level: "h2",
    }),
...
export function buildPendingSpec(query: string): RenderTreeNode {
  return node("Stack", { direction: "vertical", gap: "md" }, [
    panel(
      "Working",
      { eyebrow: "In progress", testId: "pending-state", tone: "default", size: "compact" },
      [text(query, "body"), text("Routing to the best agent...", "muted")],
    ),
  ]);
}

export function buildStreamingSpec(partialAnswer: string): RenderTreeNode {
  return node("Stack", { direction: "vertical", gap: "md" }, [
    panel("Answering", { eyebrow: "Streaming", testId: "streaming-content", tone: "default" }, [
      node("MarkdownAnswer", { content: partialAnswer, testId: null }),
    ]),
  ]);
}
...
export function buildCompletedSpec(response: AgentResponse): RenderTreeNode {
  return node("Stack", { direction: "vertical", gap: "md" }, [
    panel("Answer", { eyebrow: "Final answer", testId: "response-content", tone: "default" }, [
...
export function buildInspectorSpec(response: AgentResponse): RenderTreeNode {
  ...
  return node("Stack", { direction: "vertical", gap: "md" }, [
    panel("Tools", { eyebrow: "Execution", testId: "tools-used", tone: "default" }, [
      node("Stack", { direction: "vertical", gap: "sm" }, toolBadges),
    ]),
    panel("Reasoning", { eyebrow: "Trace", testId: "reasoning", tone: "default" }, [
